                ...
                <md-card-content>
                  <div class="md-layout">
                    ...
                    <div class="md-layout-item md-size-30">
                      ...
                      >
                        <p class="validate">*</p>
                        <label>مكان المحكمة</label>
                        <md-input v-model="CityName"></md-input>
                      </md-field>
                    </div>
        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              ...
              <md-table-cell md-label="مكان المحكمة">{{
                item.cityName
              }}</md-table-cell>
  ...
  methods: {
      ...
      try {
        ...
        if (!this.CityName) {
          return this.notifyVue(
            ...
            "center",
            "danger",
            "عفواً، الرجاء ادخال مكان المحكمة"
          );
        }
        ...
        if (this.btn === "new") {
          let result = await axios.post(`${env.HR()}/api/courts`, schema, {
            headers: { Authorization: "Bearer " + token }
          });
        } else {
          let result = await axios.put(
            `${env.HR()}/api/courts/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/courts`, {
          headers: { Authorization: "Bearer " + token }
        });
    ...
    clear() {
      (this.Name = ""), (this.CityName = ""), (this.btn = "new");
    },
    handleDelete(item) {
      Swal.fire({
