        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              <md-table-cell md-label="اسم المصرف">{{
                item.name
              }}</md-table-cell>
  ...
  methods: {
    async Save() {
      try {
        ...
        if (this.btn === "new") {
          let result = await axios.post(`${env.HR()}/api/banks`, schema, {
            headers: { Authorization: "Bearer " + token }
          });
        } else {
          let result = await axios.put(
            `${env.HR()}/api/banks/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/banks`, {
          headers: { Authorization: "Bearer " + token }
        });
    ...
    async handleEdit(item) {
      ...
      (this.Name = item.name), (this.btn = "Edit");
      this.idTable = item.id;
    },

    clear() {
      (this.Name = ""), (this.btn = "new");
    },
    handleDelete(item) {
      Swal.fire({
