  ...
  methods: {
      ...
      try {
        ...
        if (this.btn === "new") {
          ...
          let result = await axios.put(
            `${env.HR()}/api/scientific-degrees/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/scientific-degrees`, {
          headers: { Authorization: "Bearer " + token }
        });
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/scientific-degrees/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
