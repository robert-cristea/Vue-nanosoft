                  ...
                  <div class="md-layout">
                    ...
                    <div class="md-layout-item md-size-30">
                      <md-field>
                        ...
                        <md-select v-model="IsTypesBonuse">
  ...
  methods: {
    async Save() {
      try {
        ...
        let schema = {
          Name: this.Name,
          IsTypesBonuse: this.IsTypesBonuse
        };

        if (this.btn === "new") {
          let result = await axios.post(
            `${env.HR()}/api/typesBonuses`,
            schema,
            {
          ...
          let result = await axios.put(
            `${env.HR()}/api/typesBonuses/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/typesBonuses`, {
          headers: { Authorization: "Bearer " + token }
        });
    ...
    async handleEdit(item) {
      ...
      (this.Name = item.name),
        (this.IsTypesBonuse = item.isTypesBonuse),
        (this.btn = "Edit"),
        (this.idTable = item.id);
    },

    clear() {
      (this.Name = ""), (this.IsTypesBonuse = false), (this.btn = "new");
    },
    handleDelete(item) {
      Swal.fire({
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/typesBonuses/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
