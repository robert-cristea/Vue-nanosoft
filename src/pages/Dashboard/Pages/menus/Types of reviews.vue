  ...
  methods: {
      ...
      try {
        ...
        if (!this.Name) {
          return this.notifyVue(
            ...
            "center",
            "danger",
            "عفواً، الرجاء ادخال نوع التقييم"
          );
        }
        ...
        if (this.btn === "new") {
          ...
          let result = await axios.put(
            `${env.HR()}/api/evaluation-types/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/evaluation-types`, {
          headers: { Authorization: "Bearer " + token }
        });
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/evaluation-types/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
