                  ...
                  <div class="md-layout">
                    ...
                    <div class="md-layout-item md-size-30">
                      <md-field>
                        ...
                        <md-select v-model="HaveExpireDate">
                          <md-option value="true">نعم</md-option>
                          <md-option value="false">لا</md-option>
                        </md-select>
                      </md-field>
        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              ...
              <md-table-cell md-label="له تاريخ صلاحية ؟">{{
                item.haveExpireDate === false ? "لا" : "نعم"
              }}</md-table-cell>

              <md-table-cell md-label="نوعه القرار ؟">{{
                item.isDecision === false ? " لا" : "نعم"
              }}</md-table-cell>
  ...
  methods: {
    async Save() {
      try {
        ...
        let schema = {
          Name: this.Name,
          IsDecision: this.IsDecision,
          HaveExpireDate: this.HaveExpireDate
        };

        if (this.btn === "new") {
          let result = await axios.post(
            `${env.HR()}/api/document-types`,
            schema,
            {
          ...
          let result = await axios.put(
            `${env.HR()}/api/document-types/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/document-types`, {
          headers: { Authorization: "Bearer " + token }
        });
    ...
    async handleEdit(item) {
      ...
      (this.Name = item.name),
        (this.IsDecision = item.isDecision),
        (this.HaveExpireDate = item.haveExpireDate),
        (this.btn = "Edit"),
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/document-types/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
