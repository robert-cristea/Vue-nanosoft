  ...
  methods: {
      ...
      try {
        ...
        if (this.btn === "new") {
          ...
          let result = await axios.put(
            `${env.HR()}/api/Nationalities/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
    ...
    async init() {
      try {
        let T = localStorage.getItem("Nannotoken");
        let token = atob(T);
        let result = await axios.get(`${env.HR()}/api/Nationalities`, {
          headers: { Authorization: "Bearer " + token }
        });
    ...
    clear() {
      (this.Name = ""), (this.Type = ""), (this.btn = "new");
    },
    handleDelete(item) {
      Swal.fire({
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/Nationalities/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
