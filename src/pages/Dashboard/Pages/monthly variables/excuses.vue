              ...
              <div class="md-layout">
                ...
                <div class="md-layout-item md-size-30 ml-15 mtb-25">
                  <md-field>
                    ...
                    <md-select v-model="DepartmentId">
                      <md-option
        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              <md-table-cell md-label="المستهدف">{{ item.for }}</md-table-cell>

              <md-table-cell md-label="التاريخ">{{
                convert(item.date)
              }}</md-table-cell>

              <md-table-cell v-if="item.type == '1'" md-label="نوع العذر">
                حضور متأخر
              </md-table-cell>

              <md-table-cell v-if="item.type == '2'" md-label="نوع العذر">
                خروج مبكر
              </md-table-cell>

              <md-table-cell v-if="item.type == '3'" md-label="نوع العذر">
                غياب
              </md-table-cell>
  ...
  methods: {
    ...
    async Search() {
      try {
        ...
        if (this.date == null) {
          ...
          this.date = this.convert(this.date);
        }

        let result = await axios.get(
          `${env.HR()}/api/Excuses/filter?EmployeeId=${
            this.EmployeeId
          }&DepartmentId=${this.DepartmentId}&ExcuseType=${
            this.ExcuseType
          }&Date=${this.date}`,
          {
    ...
    async init() {
      try {
        ...
        this.tableData = result.data.data.excuse;
      } catch (error) {
        return this.notifyVue(
          "top",
          "center",
    ...
    New() {
      ...
      this.$router.push("Excuses/new");
    },

    handleEdit(item) {
      this.$router.push(`Excuses/${item.id}`);
    },
