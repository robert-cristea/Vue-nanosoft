        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              ...
              <md-table-cell md-label="تاريخ التقييم من">{{
                convert(item.dateFrom)
              }}</md-table-cell>

              <md-table-cell md-label="تاريخ التقييم الى">{{
                convert(item.dateTo)
              }}</md-table-cell>

              <md-table-cell md-label="المجموع">%{{ item.sum }}</md-table-cell>
...
export default {
  ...
  methods: {
    ...
    async Search() {
      try {
        ...
        let result = await axios.get(
          `${env.HR()}/api/Evaluations/filter?FullName=${
            this.FullName
          }&DateFrom=${this.DateFrom}&DateTo=${this.DateTo}&TypeOfEndService=${
    ...
    New() {
      this.$router.push("/pages/evaluation/new");
    },
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/Evaluations/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
