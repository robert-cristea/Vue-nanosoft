        ...
        <md-card-content>
          ...
          >
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              <md-table-cell md-label="رمز التصنيف">{{
                item.name
              }}</md-table-cell>
      ...
      try {
        ...
        if (!this.name) {
          return this.notifyVue(
            ...
            "center",
            "danger",
            "عفواً، الرجاء ادخال رمز التصنيف"
          );
        }
        ...
        if (this.btn === "new") {
          ...
          let result = await axios.put(
            `${env.HR()}/api/departmentClassifications/${this.idTable}`,
            schema,
            {
              headers: { Authorization: "Bearer " + token }
            }
        ...
        if (result.value) {
          try {
            ...
            let result = await axios.delete(
              `${env.HR()}/api/departmentClassifications/${item.id}`,
              {
                headers: { Authorization: "Bearer " + token }
              }
            );
